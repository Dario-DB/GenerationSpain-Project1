 ...
 * 
 * <p>
 * <strong>Descripcion </strong> clase que contiene todo los metodos
 * relacionados con peliculas
 * </p>
 * 
...
public class Catalogo implements InterfazCatalogo {
	...
	public Map<Integer, Pelicula> ordenarValoracion() {
		Map<Integer, Pelicula> resultSet = catalogo.entrySet().stream()
				.sorted(Comparator.comparingInt(e -> e.getValue().getValoracion())).collect(Collectors
						.toMap(Map.Entry::getKey, Map.Entry::getValue, (left, right) -> left, LinkedHashMap::new));
		return resultSet;
	}
	...
	public void ordenarMejores() {
		...
		ListIterator<Map.Entry<Integer, Pelicula>> iterator = new ArrayList<Map.Entry<Integer, Pelicula>>(
				resultSet.entrySet()).listIterator(resultSet.size());
		System.out.println("*** Listado de Peliculas ***");
	...
	public void ordenarMasVistas() {
		Map<Integer, Pelicula> resultSet = catalogo.entrySet().stream()
				.sorted(Comparator.comparingInt(e -> e.getValue().getVisualizaciones())).collect(Collectors
						.toMap(Map.Entry::getKey, Map.Entry::getValue, (left, right) -> left, LinkedHashMap::new));
	...
	public void ordenarPorEncimaMedia() throws CatalogoException {
		...
		ListIterator<Map.Entry<Integer, Pelicula>> iterator = new ArrayList<Map.Entry<Integer, Pelicula>>(
				resultSet.entrySet()).listIterator(resultSet.size());
		System.out.println("*** Listado de Peliculas por encima de la media ("+media+")***");
		while (iterator.hasPrevious()) {
			Map.Entry<Integer, Pelicula> entry = iterator.previous();
			Pelicula p = entry.getValue();
	...
	public float calcularMedia() {
		...
		while (peliculas.hasNext()) {
			codigo = peliculas.next();
			sumador += catalogo.get(codigo).getValoracion();
			contador++;	
		}
